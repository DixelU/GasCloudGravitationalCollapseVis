...
struct WheelVariableChanger :HandleableUIPart {
	...
	WheelVariableChanger(void(*OnApply)(double), float Xpos, float Ypos, double default_var, double default_fact, SingleTextLineSettings* STLS, string var_string = " ", string fac_string = " ", Type type = Type::exponential) : Width(100), Height(50) {
		...
		var_if = new InputField(to_string(default_var).substr(0, 8), Xpos - 25., Ypos + 15, 10, 40, STLS, nullptr, 0x007FFFFF, STLS, var_string, 8, _Align::center, _Align::center, InputField::Type::FP_Any);
		fac_if = new InputField(to_string(default_fact).substr(0, 8), Xpos - 25., Ypos - 5, 10, 40, STLS, nullptr, 0x007FFFFF, STLS, fac_string, 8, _Align::center, _Align::center, InputField::Type::FP_Any);
	}
	void Draw() override {
...
void Pause() {
	...
	switch (gc_iter::iter_pause)
	{
	case true:
		gc_iter::global_pause_lock.lock();
		break;
	case false:
		gc_iter::global_pause_lock.unlock();
		break;
	}
}
void OnSelectPropList(int ID) {
	...
	case 1:
		Field_Adapter_ptr->dsf = &gc_iter::grei_base.energy; break;
	case 2:
...
void Init() {
	...
	L->PushStrings({ "Density","Energy","Speed_x", "Speed_y","Force_x","Force_y" });
...
void mDisplay() {
	...
	if (FIRSTBOOT) {
		...
		gc_iter::grei_base.density.swap(d_dsf);
